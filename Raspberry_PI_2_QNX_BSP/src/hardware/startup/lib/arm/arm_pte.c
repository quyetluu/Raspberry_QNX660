...
#if (_PADDR_BITS-0) == 64
	// If the chip definitions were setup correctly for LPAE
	// these values are all zeroes.
	if (paddr_bits != 32) {
		if (cpu->upte_ro ||
			cpu->upte_rw ||
			cpu->kpte_ro ||
			cpu->kpte_rw ||
			cpu->mask_nc ) {
			crash("LPAE selected but pte descriptors wrong for %s",
				  armv_chip->name);
		}
	}
#endif
...
__SRCVERSION("$URL: http://svn.ott.qnx.com/product/branches/6.6.0/trunk/hardware/startup/lib/arm/arm_pte.c $ $Rev: 771846 $")
#endif
